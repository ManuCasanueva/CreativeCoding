...
class Agent {
  constructor(x, y) {
    ...
    this.radius = random.range(15, 25);
  }
...
gradient.addColorStop(0, 'rgba(255, 226, 0, 0.8)'); // Cyan oscuro
gradient.addColorStop(0.5, 'rgba(255, 104, 0, 0.8)'); // Cyan oscuro
// Cyan intermedio
gradient.addColorStop(1, 'rgba(255, 27, 27, 0.8)'); // Cyan claro
...
for (let i = 0; i < 8; i++) {
    context.lineTo(
        this.radius * Math.cos(Math.PI * i /2),
        this.radius * Math.sin(Math.PI * i / 2)
    );
}
context.closePath();
context.fill();
...
const sketch = ({ context, width, height }) => {
  ...
  return ({ context, width, height }) => {
    context.fillStyle = 'rgba(234, 240, 230, 0.81)';
